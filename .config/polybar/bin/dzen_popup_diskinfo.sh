...
[ $(($XPOS+$WIDTH)) -gt 1920 ] && XPOS=$((1920 - $WIDTH))
...
df -h | grep -E '^/dev' | while read -r F TOTAL USED AVAIL P M; do
    ...
    USE=${P%\%}
    if [ "$USE" -gt 75 ]; then
        BAR=$(echo "$USE" | dzen2-gdbar -bg $bar_bg -fg $warn -h $bar_h -w $bar_w)
    else
        BAR=$(echo "$USE" | dzen2-gdbar -bg $bar_bg -fg $bar_fg -h $bar_h -w $bar_w)
    fi
